...
Finally, the assumptions for linear regression will be checked by inspecting the residual plots for each individual linear regression. 

```{r}
par(mfrow = c(2, 3))
for (i in names(data)[-1][1:9]) {
  plot(resid(lm(get(i) ~ zeit, data)),
       ylab = "Residuals",
       main = i,
        col = "steelblue")
  abline(0, 0)
